...
resource "aws_security_group" "db" {
  ...
  description = "Allow MySQL from home IP"
  vpc_id      = aws_vpc.this.id
...
resource "aws_rds_cluster_parameter_group" "aurora_mysql" {
  name   = "${var.name_prefix}-aurora-mysql-cluster-pg"
  family = "aurora-mysql8.0"

  # --- CDC / binlog settings (common for row-based CDC tooling) ---
  parameter {
    name         = "binlog_format"
    value        = "ROW"
...
resource "aws_rds_cluster" "aurora" {
  cluster_identifier = "${var.name_prefix}-aurora-mysql"
  ...
  # IAM DB authentication (tokens) enabled at the cluster level
  iam_database_authentication_enabled = true

  # Point to the MySQL cluster parameter group
...
resource "aws_rds_cluster_instance" "aurora_instance" {
  identifier         = "${var.name_prefix}-aurora-mysql-1"
  ...
  tags = {
    Name = "${var.name_prefix}-aurora-mysql-1"
  }
}
...
resource "local_file" "iam_policy_json" {
  filename        = "${path.module}/${var.name_prefix}-generated_x-account-rds-iam-policy.json"
